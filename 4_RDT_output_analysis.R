...
for(dir in RDT_outputs){
    ...
    rdt <- ChemmineR::read.SDFindex(rxn,
                                    index = data.frame(
                                        "A" = which(stringr::str_detect(rdt_lines, "(CHEBI|POLYMER):[:digit:]{1,6}")),
                                        "B" = which(stringr::str_detect(rdt_lines, "END"))
                                    ))
    
    react_id <- stringr::str_extract(rdt_lines[3], "[:digit:]{5}")
    ...
    if(any(sapply(ChemmineR::atomblock(rdt), dim)[2, ] == 2)){
        ...
        rdt <- ChemmineR::read.SDFindex(rxn,
                                        index = data.frame(
                                            "A" = which(stringr::str_detect(rdt_lines, "(CHEBI|POLYMER):[:digit:]{1,6}"))[-hydro_pos],
